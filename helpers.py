...
class postgres():
	...
	def execute(self, q, commit=True):
		log('SQLExecute: {q} [{commit}]'.format(q=q, commit=commit), level=1, product='slimSMTP', handler='postgresql')
		self.cur.execute(q)
		if commit:
			log('Commited!', level=1, product='slimSMTP', handler='postgresql')
			self.con.commit()

			#log(list(self.query('SELECT * FROM access_tokens;')), level=1)

	def query(self, q, commit=False):
		log('SQLQuery: {q} [{commit}]'.format(q=q, commit=commit), level=1, product='slimSMTP', handler='postgresql')
		self.cur.execute(q)
		if commit:
			self.con.commit()
...
def log(*args, **kwargs):
	if dCheck(config, 'log'):
		if not '_logstream' in runtime:
			runtime['_logstream'] = journal.stream('slimSMTP')
